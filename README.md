...
## Nylas API v2 vs v3
The `main` branch of the repo now leverages Nylas API v3 (now GA).  The `v2` branch of this repo will track Nylas API v2 (now deprecated), though dev work on this app will focus on Nylas API v3.

## **Note**
_As of API v3, Nylas parsed contacts are 'generated' by the provider (Google/Microsoft) in contrast to API v2, which parsed email interactions and generated contacts.  At present, webhooks are not supported for these types of contacts in API v3, thus the app now uses messages to create parsed contacts within the app._
