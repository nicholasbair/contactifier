defmodule ContactifierWeb.IntegrationController do
  ...
  def callback(conn, params) do
    with {:ok, account} <- ContactProvider.exchange_code_for_token(params["code"]),
          {:ok, integration} <- Contactifier.Integrations.upsert_integration(%{"name" => "Email/Contacts", "scopes" => ["email.read_only", "contacts.read_only"], "valid?" => true, "user_id" => conn.assigns.current_user.id, "token" => account.access_token, "vendor_id" => account.account_id, "invalid_since" => nil}) do
